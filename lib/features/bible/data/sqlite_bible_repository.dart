...
class SqliteBibleRepository implements BibleRepository {
  ...
  String _bookName(dynamic row, String id) => switch (id) {
    'cuv_hant' => row['name_zh_hant'] as String,
    'cuv_hans' => row['name_zh_hans'] as String,
    _ => row['name_en'] as String,
  };

  BibleVerse _map(dynamic row, String translationId) => BibleVerse(
    id: row['id'] as int,
  ...
  String _select(String translationId) =>
      '''
    SELECT v.id, v.book_id, b.name_en, b.name_zh_hant, b.name_zh_hans,
           v.chapter, v.verse, v.$_column(translationId) AS text,
           t.id AS translation_id, t.name AS translation_name,
           t.language_code
    FROM verses v
  ...
  Future<List<BibleTranslation>> getTranslations() async {
    final rows = _db.select(
      'SELECT id, name, language_code FROM translations ORDER BY id',
    );
    return rows
        .map(
          (r) => BibleTranslation(
  ...
  Future<BibleVerse?> getVerse({
    ...
    final rows = _db.select(
      '${_select(translationId)} WHERE v.book_id=? AND v.chapter=? AND v.verse=? LIMIT 1',
  ...
  Future<List<BibleVerse>> getChapter({
    ...
    final rows = _db.select(
      '${_select(translationId)} WHERE v.book_id=? AND v.chapter=? ORDER BY v.verse',
      [translationId, bookId, chapter],
    );
    return rows.map((r) => _map(r, translationId)).toList(growable: false);
  }
  ...
  Future<List<BibleVerse>> getRange({
    ...
    final rows = _db.select(
      '${_select(translationId)} WHERE v.book_id=? AND v.chapter=? AND v.verse BETWEEN ? AND ? ORDER BY v.verse',
      [
        translationId,
        reference.bookId,
        reference.chapter,
  ...
  Future<List<BibleVerse>> search({
    ...
    final rows = _db.select(
      ...
      WHERE t.id=? AND f.text MATCH ?
      LIMIT ?
      ''',
      [translationId, query.replaceAll('"', '""'), limit],
    );
    return rows.map((r) => _map(r, translationId)).toList(growable: false);
  }
}
